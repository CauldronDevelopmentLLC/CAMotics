...
STLReader::STLReader(const InputSource &source) :
...
uint32_t STLReader::readHeader(string &name, string &hash) {
  char buffer[1024];
...
bool STLReader::hasMore() {
  return !stream.fail() &&
    ((binary && count) || (!binary && parser.check("facet")));
}


void STLReader::readFacet(Vector3F &v1, Vector3F &v2, Vector3F &v3,
                          Vector3F &normal) {
  if (binary) {
